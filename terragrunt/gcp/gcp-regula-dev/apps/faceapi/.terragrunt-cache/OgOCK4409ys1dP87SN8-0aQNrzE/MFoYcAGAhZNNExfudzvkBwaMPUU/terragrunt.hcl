...
dependency "gke" {
  config_path = "../../infra/gke/gke-cluster"
  mock_outputs = {
...
inputs = {
  database_connection_string = "postgresql://${dependency.gsql.outputs.additional_users[1].name}:${dependency.gsql.outputs.additional_users[1].password}@${dependency.gsql.outputs.private_ip_address}:5432/faceapi"
